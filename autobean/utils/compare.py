...
def compare_entries(entries1: List[Directive], entries2: List[Directive]):
    hashes1 = {hash_entry(entry): entry for entry in entries1}
    hashes2 = {hash_entry(entry): entry for entry in entries2}
    keys1 = Counter(hash_entry(entry) for entry in entries1)
    keys2 = Counter(hash_entry(entry) for entry in entries2)
