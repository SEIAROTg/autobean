...
class Testbalance(base.BaseTestModel):
    ...
    def test_parse_success(
        ...
        account = 'Assets:Foo'
        balance = self.parser.parse(text, models.Balance)
    ...
    def test_parse_failure(self, text: str) -> None:
        with pytest.raises(exceptions.UnexpectedInput):
            self.parser.parse(text, models.Balance)

    def test_set_raw_date(self) -> None:
        balance = self.parser.parse('2000-01-01  balance Assets:Foo 100.00 USD', models.Balance)
    ...
    def test_set_raw_number(self) -> None:
        balance = self.parser.parse('2000-01-01 balance Assets:Foo  100.00  USD', models.Balance)
        new_number = models.NumberExpr.from_value(_D('-123.45'))
    ...
    @pytest.mark.parametrize(
        'text,raw_tolerance,expected', [
            ...
            ('2000-01-01 balance Assets:Foo 100.00  USD',
             models.Tolerance.from_children(models.NumberExpr.from_value(_D('0.01'))),
             '2000-01-01 balance Assets:Foo 100.00 ~ 0.01  USD'),
            ('2000-01-01 balance Assets:Foo 100.00 ~  0.01   USD',
             models.Tolerance.from_children(models.NumberExpr.from_value(_D('0.02'))),
    ...
    def test_set_raw_tolerance(self, text: str, raw_tolerance: Optional[models.Tolerance], expected: str) -> None:
        balance = self.parser.parse(text, models.Balance)
    ...
    def test_from_children_with_tolerance(self) -> None:
        ...
        number = models.NumberExpr.from_value(_D('100.00'))
        tolerance = models.Tolerance.from_children(models.NumberExpr.from_value(_D('0.01')))
        currency = models.Currency.from_value('USD')
        balance = models.Balance.from_children(date, account, number, tolerance, currency)
    ...
    def test_from_children_without_tolerance(self) -> None:
        ...
        number = models.NumberExpr.from_value(_D('100.00'))
        currency = models.Currency.from_value('USD')
        balance = models.Balance.from_children(date, account, number, None, currency)
        assert balance.raw_date is date
        assert balance.raw_account is account
        assert balance.raw_number is number
    ...
    def test_from_value_with_tolerance(self) -> None:
        balance = models.Balance.from_value(
