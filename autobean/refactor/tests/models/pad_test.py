...
class TestPad(base.BaseTestModel):
    ...
    def test_parse_success(
        ...
        pad = self.easy_parser.parse(text, easy_models.Pad)
    ...
    def test_parse_failure(self, text: str) -> None:
        with pytest.raises(exceptions.UnexpectedInput):
            self.raw_parser.parse(text, raw_models.Pad)

    def test_set_raw_date(self) -> None:
        pad = self.raw_parser.parse('2000-01-01  pad Assets:Foo Assets:Bar', raw_models.Pad)
    ...
    def test_set_date(self) -> None:
        pad = self.easy_parser.parse('2000-01-01  pad Assets:Foo Assets:Bar', easy_models.Pad)
