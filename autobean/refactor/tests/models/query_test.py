...
class TestQuery(base.BaseTestModel):
    ...
    def test_parse_failure(self, text: str) -> None:
        with pytest.raises(exceptions.UnexpectedInput):
            self.parser.parse(text, models.Query)

    def test_set_raw_date(self) -> None:
        query = self.parser.parse('2000-01-01  query "foo" "bar"', models.Query)
    ...
    def test_set_raw_query_string(self) -> None:
        query = self.parser.parse('2000-01-01 query "foo"  "bar"', models.Query)
        new_query_string = models.EscapedString.from_value('baz')
    ...
    def test_from_children(self) -> None:
        ...
        query_string = models.EscapedString.from_value('bar')
        query = models.Query.from_children(date, type, query_string)
    ...
    def test_from_value(self) -> None:
        query = models.Query.from_value(datetime.date(2012, 12, 12), 'foo', 'bar')
        assert query.date == datetime.date(2012, 12, 12)
        assert query.name == 'foo'
        assert query.query_string == 'bar'
        assert self.print_model(query) == '2012-12-12 query "foo" "bar"'
