...
class TestClose(base.BaseTestModel):
    ...
    def test_parse_success(
        ...
        close = self._parser.parse(text, easy_models.Close)
    ...
    def test_parse_failure(self, text: str) -> None:
        with pytest.raises(exceptions.UnexpectedInput):
            self._parser.parse(text, raw_models.Close)

    def test_set_raw_date(self) -> None:
        close = self._parser.parse('2000-01-01  close Assets:Foo', raw_models.Close)
    ...
    def test_set_date(self) -> None:
        close = self._parser.parse('2000-01-01  close Assets:Foo', easy_models.Close)
