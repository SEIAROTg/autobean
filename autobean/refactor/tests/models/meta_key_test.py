...
class TestLink(base.BaseTestModel):
    ...
    def test_parse_success(self, text: str, value: str) -> None:
        token = self.raw_parser.parse_token(text, raw_models.MetaKey)
    ...
    def test_parse_failure(self, text: str) -> None:
        with pytest.raises(exceptions.UnexpectedInput):
            self.raw_parser.parse_token(text, raw_models.MetaKey)

    def test_set_raw_text(self) -> None:
        token = self.raw_parser.parse_token('foo:', raw_models.MetaKey)
        token.raw_text = 'bar:'
